...
public final class MuesliController: NSObject {
    ...
    func canUseSummaryProvider(_ provider: MeetingSummaryBackendOption) -> Bool {
        switch provider {
        case .chatGPT: return chatGPTAuth.isAuthenticated
        case .openAI: return !resolvedOpenAIAPIKey().trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        case .openRouter:
            return !openRouterAuth.resolvedAPIKey(legacyAPIKey: config.openRouterAPIKey).isEmpty
    ...
    func resummarize(meeting: MeetingRecord, selection: MeetingSummarySelection? = nil, completion: @escaping (Result<Void, Error>) -> Void) {
        let templateSnapshot = meetingTemplateSnapshot(for: meeting)
        resummarize(meeting: meeting, using: templateSnapshot, selection: selection, completion: completion)
    }

    func applyMeetingTemplate(id: String, to meeting: MeetingRecord, selection: MeetingSummarySelection? = nil, completion: @escaping (Result<Void, Error>) -> Void) {
    ...
    private func resummarize(
        ...
        selection: MeetingSummarySelection?,
        completion: @escaping (Result<Void, Error>) -> Void
    ) {
        let summaryConfig = selection?.applying(to: config) ?? config
